...
import json
from base.prodserv_models import Service, Product

__author__ = 'amado'


def get_products(business):
        products = business.product_set.order_by('category')
        # for p in products:
        #     p.category_name = p.category.description
        ...
        if business:
            ...
            return render_to_response('base/booking/booking_widget.html', RequestContext(request,
                                                                                 {'business':business,
                                                                                  'wskb64':key,
                                                                                  'form':form,
                                                                                  'services':get_services(business),
                                                                                  'products':get_products(business),
                                                                                   'states':states
                                                                                  }))
        else:
